---
title: "Upgrading from PHPStan 1.x to 2.0"
date: 2024-11-11
tags: guides
---

This is the upgrading guide for [PHPStan 2.0](/blog/phpstan-2-0-released-level-10-elephpants).
...
and enable [**Bleeding Edge**](/blog/what-is-bleeding-edge). This will enable the new rules and behaviours that 2.0 turns on for all users.
...
Don't forget to update [3rd party PHPStan extensions](/user-guide/extension-library) as well.

After changing your `composer.json`, run `composer update 'phpstan/*' -W`.

It's up to you whether you go through the new reported errors or if you just put them all to the [baseline](/user-guide/baseline) ;) Everyone who's on PHPStan 1.12 should be able to upgrade to PHPStan 2.0.

### Noteworthy changes to code analysis

* [**Enhancements in handling parameters passed by reference**](/blog/enhancements-in-handling-parameters-passed-by-reference)
* [**Validate inline PHPDoc `@var` tag type**](/blog/phpstan-1-10-comes-with-lie-detector#validate-inline-phpdoc-%40var-tag-type)
* [**List type enforced**](/blog/phpstan-1-9-0-with-phpdoc-asserts-list-type#list-type)
...
It has been replaced with [`excludePaths`](/user-guide/ignoring-errors#excluding-whole-files).
...
Learn more: [Using RuleErrorBuilder to enrich reported errors in custom rules](/blog/using-rule-error-builder)
...
Using `$type instanceof ArrayType` is [being deprecated anyway](/blog/why-is-instanceof-type-wrong-and-getting-deprecated) so the impact of this change should be minimal.

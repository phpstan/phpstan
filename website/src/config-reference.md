...
When set to `true` (the default), PHPStan reports [`throws.unusedType`](/error-identifiers/throws.unusedType) errors for exception types written in an explicit `@throws` PHPDoc tag that are never actually thrown by the function, method, or property hook. This check is enabled at [rule level](/user-guide/rule-levels) 4 and up.
...
Set to `false` to turn this check off. To also report too-wide `@throws` types that were inherited or implicitly inferred (not just those explicitly written in PHPDoc), see [`exceptions.check.tooWideImplicitThrowType`](#exceptionschecktoowideimplicitthrowtype).

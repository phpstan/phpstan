...
`template-type` can be used to [get `@template` type from a passed object argument](/r/ceb59974-0a7c-492a-867a-5d5b7c30e52f). Related discussion [here](https://github.com/phpstan/phpstan/discussions/9053).

`new` can be used to [create an object type from a class-string type](/r/a01e1e49-6f05-43a8-aac7-aded770cd88a).
...
* `static-closure(int, int): string` (Closure declared as `static`, so it doesn't bind `$this`)
* `static-pure-closure(int, int): string` (static Closure that also doesn't have any side effects when called)

...
The same applies to writes to an array offset, such as `$a['x'] = 1;` followed by `$a['x'] = 2;` or `$a = [];` before `$a['x']` is read.

Unlike [`variable.unused`](/error-identifiers/variable.unused), the variable itself *is* used later; only this particular assignment is redundant. Unlike [`assign.unused`](/error-identifiers/assign.unused), another assignment replaces the value before it can be read. Dead stores like this often point to a logic error, such as forgetting to use the first value or an accidental overwrite.

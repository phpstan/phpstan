...
2. **`npm run build:parcel`** — Parcel bundles and optimizes everything from `tmp/` into `dist/`
...
- `dist/` — Final build output from Parcel (gitignored)

### Scripts

- `npm run build` — Full production build (11ty + Parcel)
- `npm run watch` — Dev mode (runs 11ty watch + Parcel dev server in parallel)

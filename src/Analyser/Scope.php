...
class Scope
{
	...
	private function resolveType(Expr $node): Type
	{
		...
		if ($node instanceof LNumber) {
			...
			foreach ($node->params as $param) {
				...
				$parameters[] = new NativeParameterReflection(
					$param->var->name,
					$optional,
					$this->getFunctionType($param->flags, $param->flags === null, $param->variadic),
					$param->byRef
			...
			$closureReturnType = isset($node->returnType) ? $node->returnType->name ?? null : null;

			return new ClosureType(
				$parameters,
				$this->getFunctionType($node->returnType, $closureReturnType === null, false),
				$isVariadic
			);
		...
		} elseif ($node instanceof Node\Expr\ClassConstFetch && is_string($node->name->name)) {
			if ($node->class instanceof Name) {
		...
		if ($node instanceof MethodCall && is_string($node->name->name)) {
		...
		if ($node instanceof Expr\StaticCall && is_string($node->name->name)) {
			if ($node->class instanceof Name) {
		...
		if ($node instanceof PropertyFetch && is_string($node->name->name)) {
		...
		if ($node instanceof Expr\StaticPropertyFetch && is_string($node->name->name) && $node->class instanceof Name) {
	...
	): self
	{
		...
		$classMethodReturnType = isset($classMethod->returnType) ? $classMethod->returnType->name ?? null : null;
		...
		return $this->enterFunctionLike(
			new PhpMethodFromParserNodeReflection(
				...
				$classMethodReturnType !== null,
				$this->getFunctionType($classMethod->returnType, $classMethodReturnType === null, false),
				$phpDocReturnType
			)
		);
	}
	...
	{
		$functionReturnType = isset($function->returnType) ? $function->returnType->name ?? null : null;
		...
		return $this->enterFunctionLike(
			new PhpFunctionFromParserNodeReflection(
				...
				$this->getFunctionType($function->returnType, $functionReturnType === null, false),
				$phpDocReturnType
			)
		);
	}
	...
	public function getFunctionType($type = null, bool $isNullable, bool $isVariadic): Type
	{
		...
		if ($type === null) {
			...
			return $this->getFunctionType($typeName, true, $isVariadic);
		}

...
class NodeScopeResolver
{
    ...
    private function processNode(\PhpParser\Node $node, Scope $scope, \Closure $nodeCallback, bool $stopImmediately = false): void
    {
        ...
        if ($node instanceof \PhpParser\Node\Stmt\ClassLike
            ...
            if (!(new ObjectType(\Closure::class))->isSuperTypeOf($scope->getType($node->var))->no()
            ...
            foreach ($node->vars as $unsetVar) {
                while ($unsetVar instanceof ArrayDimFetch
        ...
        foreach ($node->getSubNodeNames() as $subNodeName) {
            ...
            if (is_array($subNode)) {
                ...
                if (($node instanceof BooleanAnd || $node instanceof BinaryOp\LogicalAnd)
                    && $subNodeName === 'right') {
                    $scope = $scope->filterByTruthyValue($node->left);
                }
                if (($node instanceof BooleanOr || $node instanceof BinaryOp\LogicalOr)
                    && $subNodeName === 'right') {
    ...
    private function ensureNonNullability(
        ...
        $nodeToSpecify = $node;
        while ($nodeToSpecify instanceof PropertyFetch
            ...
            if ($nodeToSpecify instanceof PropertyFetch
                || $nodeToSpecify instanceof MethodCall
            ) {
    ...
    private function lookForAssigns(
        ...
        if ($node instanceof StaticVar) {
            ...
            if ($node instanceof FuncCall
                ...
                } elseif ($functionName === 'array_unshift'

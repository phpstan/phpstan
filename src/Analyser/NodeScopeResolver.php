...
class NodeScopeResolver
{
		...
		foreach ($nodes as $i => $node) {
			...
			if (
				...
				if ($parametersAcceptor !== null) {
					...
					} elseif ($parametersAcceptor->isVariadic()) {
	...
	private function specifyFetchedPropertyForInnerScope(
		Node $node,
		Scope $inScope,
		bool $inEarlyTermination,
		Scope &$scope
	): void
	{
	...
	private function specifyItemFromArrayDestructuring(
		Scope $scope,
		ArrayItem $arrayItem,
		Type $valueType,
		Type $keyType
	): Scope
	{
		...
		if (
			$itemNode instanceof Variable
			&&
			\is_string($itemNode->name)
		) {

			$scope = $scope->assignVariable($itemNode->name, $type, TrinaryLogic::createYes());

		} elseif (
			$itemNode instanceof ArrayDimFetch
			&&
			$itemNode->var instanceof Variable
			&&
			\is_string($itemNode->var->name)
		) {
	...
	private function enterForeach(Scope $scope, Foreach_ $node): Scope
	{
		...
		if (
			$node->valueVar instanceof Variable
			&&
	...
	): void
	{
		...
		if ($node instanceof \PhpParser\Node\Stmt\ClassLike) {
			...
			if (isset($node->args[1])) {
				...
				if (
					$argValue instanceof Expr\ConstFetch
					&&
					(string) $argValue->name === 'null'
				) {
					$thisType = null;
				} else {
			...
			foreach ($node->cond as $condExpr) {
				$scopeLoopMightHaveRun = $this->lookForAssigns(
			...
			if ($node->finally !== null) {
				$finallyScope = $this->lookForAssignsInBranches(
					$scopeForLookForAssignsInBranches,
					$statements,
			...
			if (\is_array($subNode)) {
				...
				if (
					(
						...
						||
						$node instanceof BinaryOp\LogicalAnd
					)
					&&
					$subNodeName === 'right'
				) {
				...
				if (
					(
						...
						||
						$node instanceof BinaryOp\LogicalOr
					)
					&&
					$subNodeName === 'right'
				) {
				...
				if (
					...
					if ($subNodeName === 'if') {
						...
						$this->processNode(
							$node->cond,
							$scope,
							function (Node $node, Scope $inScope) use (&$scope): void {
								$this->specifyFetchedPropertyForInnerScope($node, $inScope, false, $scope);
							}
						);
				...
				if ($scope->isInFirstLevelStatement()) {
					if (
						...
						(
							$node instanceof BooleanAnd
							||
							$node instanceof BinaryOp\BooleanOr
						)
						&&
						$subNodeName === 'right'
					) {
	...
	private function lookForEnterVariableAssign(Scope $scope, Expr $node): Scope
	{
		if ($node instanceof Variable) {
			...
			$node instanceof Assign
			&&
			$node->expr instanceof Expr\Closure
		) {
	...
	): Scope
	{
		...
		if ($node instanceof StaticVar) {
			...
			$closureScope = $this->lookForAssignsInBranches(
				$scope,
				$statements,
				LookForAssignsSettings::insideClosure()
			);
	...
	): Scope
	{
		...
		foreach ($statementsLists as $statementList) {
			...
			foreach ($statements as $statement) {
				$branchScope = $this->lookForAssigns(
					$branchScope,
					$statement,
					TrinaryLogic::createYes(),
	...
	private function findParametersAcceptorInFunctionCall(
		Expr $functionCall,
		Scope $scope
	...
	private function enterClassMethod(Scope $scope, Node\Stmt\ClassMethod $classMethod): Scope
	{
		[
			...
			$isInternal,
			$isFinal,
		] = $this->getPhpDocs($scope, $classMethod);
	...
	private function enterFunction(Scope $scope, Node\Stmt\Function_ $function): Scope
	{
		[
			...
			$isInternal,
			$isFinal,
		] = $this->getPhpDocs($scope, $function);

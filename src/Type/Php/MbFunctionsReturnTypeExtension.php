...
class MbFunctionsReturnTypeExtension implements \PHPStan\Type\DynamicFunctionReturnTypeExtension
{
	...
	{
		$this->supportedEncodings = array_map('strtoupper', mb_list_encodings());
	}
	...
	public function getTypeFromFunctionCall(FunctionReflection $functionReflection, FuncCall $functionCall, Scope $scope): Type
	{
		...
		if (!$returnType instanceof UnionType || !\extension_loaded('mbstring')) {
			return $returnType;
		}
		...
		}, TypeUtils::getConstantStrings($scope->getType($functionCall->args[$positionEncodingParam - 1]->value))));

		if ($returnType->contains(new BooleanType())) {
